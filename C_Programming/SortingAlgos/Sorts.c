...
{ 
    ...
    for (i = 1; i < n; i++) { 
        key = arr[i]; 
        hole = i-1;
        /* Move elements of arr[0..hole], that are 
...
void MergeSort(int *A, int n) {
	...
	// L and R are left and right subarrays
	// `mid` elements (from index 0 till mid-1) should be part of left sub-array 
	// and `n-mid` elements (from mid to n-1) will be part of right sub-array
	int* L = (int*)malloc(mid*sizeof(int)); 
	int* R = (int*)malloc((n-mid)*sizeof(int)); 
	
	for(i=0; i<=(mid-1); i++) L[i] = A[i]; // creating left subarray
	for(i=mid; i<=(n-1); i++) R[i-mid] = A[i]; // creating right subarray
